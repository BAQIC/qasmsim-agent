...
use emulate::{pre_process_msg, EmulateInfo, EmulateMessage, EmulateMode};
...
pub async fn quantum_thread(
    ...
    match msg.mode {
        ...
        // for vqe mode, need expectation value
...
pub async fn classical_thread(
    ...
    match mode {
        ...
        | EmulateMode::Sequence => {
            ...
            match res_rx.await {
                Ok(Ok(result)) => {
                    ...
                    match emulate::post_process_msg(
                        result.sequences().clone().unwrap(),
                        mode.to_string(),
                    ) {
        ...
        EmulateMode::Vqe => {
            for n in 0..msg.iterations.unwrap_or(1) {}
            (
                StatusCode::BAD_REQUEST,
                Json(json!({"Error": "VQE mode not supported"})),
            )
        }
    }
}
